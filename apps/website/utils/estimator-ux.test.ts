...
describe("estimator UX helpers", () => {
  it("sorts market values while keeping unavailable markets last", () => {
    const markets = [
      { id: "unavailable", isAvailable: false, value: null, originalIndex: 0 },
      { id: "middle", isAvailable: true, value: 1.3, originalIndex: 1 },
      { id: "high", isAvailable: true, value: 2, originalIndex: 2 },
      { id: "low", isAvailable: true, value: 1.1, originalIndex: 3 },
    ];

    expect(
      markets
        .sort((left, right) => compareMarketSortValues(left, right, "desc"))
        .map((market) => market.id),
    ).toEqual(["high", "middle", "low", "unavailable"]);
    expect(
      markets
        .sort((left, right) => compareMarketSortValues(left, right, "asc"))
        .map((market) => market.id),
    ).toEqual(["low", "middle", "high", "unavailable"]);
  });

  it("translates health factor into a compact collateral-value buffer", () => {
    expect(liquidationBufferDescription(2)).toBe(
      "Collateral value could fall about 50% before liquidation.",
    );
    expect(liquidationBufferDescription(1.6)).toBe(
      "Collateral value could fall about 37.5% before liquidation.",
    );
    expect(liquidationBufferDescription(1.005)).toBe(
      "Collateral value could fall about 0.5% before liquidation.",
    );
    expect(liquidationBufferDescription(1)).toBe(
      "At or beyond the liquidation threshold.",
    );
    expect(liquidationBufferDescription(null)).toBe(
      "Available when this market can support the amount.",
    );
  });

  it("keeps adjacent liquidation buffers visibly distinct", () => {
    expect(liquidationBufferDescription(1.33)).toBe(
      "Collateral value could fall about 24.8% before liquidation.",
    );
    expect(liquidationBufferDescription(1.34)).toBe(
      "Collateral value could fall about 25.4% before liquidation.",
    );
  });

  it("shows a request shortfall once a minimum-constrained request reaches the market minimum", () => {
    expect(minimumConstrainedMarketStatusLabel(200, 100, 28.88)).toBe(
      "Short by $171.12",
    );
    expect(minimumConstrainedMarketStatusLabel(50, 100, 28.88)).toBe(
      "Max below $100.00 minimum",
    );
  });
  ...
  it("normalizes effective APY to current borrow APR", () => {
    expect(annualRateAsApr(Math.expm1(0.05), "apy")).toBeCloseTo(0.05, 10);
    expect(
      providerBorrowApr({
        annualRate: {
          value: Math.expm1(0.065),
          convention: "apy",
          rateType: "variable",
          sourceId: "test",
        },
  ...
  it("weights Morpho borrow APR across every active route leg", () => {
    expect(
      weightedRouteBorrowApr([
        {
          borrowAmount: { raw: "100000000", decimals: 6 },
          currentBorrowApy: {
            numerator: String(Math.round(Math.expm1(0.05) * 1e12)),
            denominator: "1000000000000",
          },
        },
        {
          borrowAmount: { raw: "300000000", decimals: 6 },
          currentBorrowApy: {
            numerator: String(Math.round(Math.expm1(0.1) * 1e12)),
            denominator: "1000000000000",
          },
        },
      ]),
    ).toBeCloseTo(0.0875, 8);
  });
