...
**The Health Factor Signal.** Present health factor as a compact liquidation-buffer summary with a semantic color dot, the exact numeric value, and a plain-language estimate of how far collateral value could fall before liquidation. Use moss at 1.60 and above, ochre from 1.20 to 1.59, and terracotta below 1.20. A value at or below 1.00 is at or beyond the liquidation threshold. These are transparent proximity bands, never a recommendation or probability.
...
- **Subtitle** (650, 1.125rem, 1.5rem line-height, -0.0125em): market identity and compact subsection headings.
...
The comparison stage becomes a workbench from 1280px upward: a 320px request controller stays sticky in the left column while the market evidence field receives the wider right column. Amount and range controls stack inside the rail, keeping the request visible while market evidence grows vertically. Below 1280px, the controller returns to a full-width horizontal composition where space permits and remains ahead of markets in both visual and DOM order.
...
Market summaries remain aligned and visible for scanning. One market at a time may disclose its technical evidence beneath the summary row, using a bounded 240ms expansion and a 160ms close.

**The Motion Carries State Rule.** Animate only wallet state, active scanning, the quiet workflow handoff, and explicit market disclosure. Static evidence and repeated market rows do not receive decorative entrances.
...
On the comparison workbench, the amount field and range form one sticky request controller at wide widths. The controller never competes with market evidence for horizontal space before 1280px; at intermediate widths its internal field and range may sit side by side, while at wide and phone widths they stack.
...
Protocol paths share the same compact 12px-radius bordered-row structure. Each collapsed row aligns market identity and availability with annual rate, available capacity, and liquidation buffer so several markets can be scanned without scrolling. One row opens at a time to reveal LTV, LLTV, liquidation thresholds, route mechanics, asset eligibility, source evidence, and the destination action without changing the summary columns. Multi-route markets expose their links alongside each route.
