...
function parseEditableAmount(value: string) {
  if (value === "" || value === ".") {
    return {
      ok: true as const,
      amount: { raw: "0", decimals: amount.value.decimals },
    };
  }
  if (!/^\d*(?:\.\d*)?$/.test(value)) {
    return {
      ok: false as const,
      message: "Enter numbers and one decimal point only.",
    };
  }
  ...
  const normalized = value.startsWith(".")
    ...
    : value.endsWith(".")
      ? `${value}0`
      : value;
  return parseUsdcAmount(normalized);
}
...
function onBeforeTextInput(event: InputEvent): void {
  ...
  const parsed = parseEditableAmount(candidate);
  if (parsed.ok) return;
          ...
          <button
            ...
            :class="
              isSelectedUtilization(target)
                ...
                : 'text-ink hover:bg-surface hover:text-river'
            "
