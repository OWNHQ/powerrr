...
test("browser tab uses the Powerrr product identity", async ({ page }) => {
  await page.goto("./");

  await expect(page).toHaveTitle("Powerrr");
  await expect(page.locator('link[rel="icon"]')).toHaveAttribute(
    "href",
    "./favicon.svg",
  );

  const faviconUrl = new URL("./favicon.svg", page.url()).toString();
...
test("wallet read disclosure opens without shifting the landing layout", async ({
  ...
  await expect(
    page.getByRole("heading", {
      name: "See how much you could borrow.",
    }),
  ).toBeVisible();
  await expect(
    page.getByText(
      "Connect a wallet to see your usable collateral across different lending markets, compare borrowing terms, and review risk.",
    ),
  ).toBeVisible();
  await expect(page.getByText("Read-only connection")).toBeVisible();
  await expect(
    page.getByText("No signature or transaction", { exact: true }),
  ).toBeVisible();
  await expect(page.getByText("Ethereum mainnet")).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Check my wallet" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(/checks the tokens in your wallet to estimate how much/i),
  ).toBeVisible();
  await expect(
    page.getByText("Technical details", { exact: true }),
  ).toBeVisible();
  await expect(page.getByText(/chunked Multicall3 read/i)).toBeVisible();
...
test("static wallet scan is explicit and uses no Powerrr API", async ({
  ...
  await expect(
    collateralCoverage.getByText(
      "Some market sources were unavailable, so pooled coverage may be understated.",
    ),
  ).toBeVisible();
  ...
  await expect(
    page
      .locator('[data-protocol-id="aave"]')
      .getByText("5.00%", { exact: true })
      .first(),
  ).toBeVisible();
  await expect(page.getByText("Projected LTV", { exact: true })).toHaveCount(0);
  for (const protocolId of ["aave", "sparklend"]) {
    const card = page.locator(`[data-protocol-id="${protocolId}"]`);
    await expect(card.getByText("APR", { exact: true })).toBeVisible();
    await expect(card.getByText("Max borrow", { exact: true })).toBeVisible();
    const bufferSummary = card.locator("[data-health-factor]");
    await expect(bufferSummary).toBeVisible();
    await expect(bufferSummary.locator("[data-risk-indicator]")).toBeVisible();
    await expect(bufferSummary.getByLabel(/Health factor/)).toBeVisible();
    await expect(bufferSummary).toContainText(
      /Collateral value could fall about \d+(?:\.\d+)?% before liquidation\./,
    );
  }
  const borrowAmount = page.getByLabel("Borrow amount in USDC");
  await borrowAmount.fill("6649.28129");
  await page
    .getByRole("heading", { name: "Compare borrowing paths", exact: true })
    .click();
  await expect(borrowAmount).toHaveValue("6649.28");
  await borrowAmount.fill("");
  await expect(borrowAmount).toHaveValue("");
  await expect(borrowAmount).toHaveAttribute("aria-invalid", "false");
  await expect(page.locator("#amount-error")).toHaveCount(0);
  await borrowAmount.fill("12");
  await borrowAmount.press("a");
  await expect(borrowAmount).toHaveValue("12");
  await expect(borrowAmount).toHaveAttribute("aria-invalid", "true");
  await expect(
    page.getByText("Enter numbers and one decimal point only."),
  ).toBeVisible();
  await borrowAmount.press("Backspace");
  await expect(borrowAmount).toHaveValue("1");
  await expect(borrowAmount).toHaveAttribute("aria-invalid", "false");
  await expect(page.locator("#amount-error")).toHaveCount(0);
  await borrowAmount.fill("");
  await expect(page.locator("#amount-error")).toHaveCount(0);
...
test("pooled risk responds to the selected amount without an always-green state", async ({
  ...
  const aaveAction = aave.locator("[data-provider-action]");
  ...
  const liquidationBuffer = aave.locator("[data-health-factor]");
  await expect(liquidationBuffer.locator("[data-risk-indicator]")).toHaveClass(
    /bg-moss/,
  );
  await expect(liquidationBuffer).toContainText(
    /Collateral value could fall about \d+(?:\.\d+)?% before liquidation\./,
  );
  await expect(liquidationBuffer.locator("[data-risk-zone]")).toHaveCount(0);
  const compoundAction = page
    .locator('[data-protocol-id="compound-iii"]')
    .locator("[data-provider-action]");
  await page
    .locator('[data-protocol-id="compound-iii"]')
    .getByRole("button")
    .first()
  ...
  await aave.getByRole("button").first().click();
  await expect(
    aave.getByRole("heading", { name: "Technical details" }),
  ).toBeVisible();
  await expect(aave.getByText("Projected LTV", { exact: true })).toBeVisible();
  await expect(
  ...
  await expect(
    aave.locator("a", {
      hasText: "Review this option on Aave Ethereum Core V3",
    }),
  ).toHaveAttribute("aria-disabled", "true");
  await amountSlider.fill("1000");
  const aaveMarketLink = aave.getByRole("link", {
    name: "Review this option on Aave Ethereum Core V3",
  });
...
test("market technical details use a single-open disclosure", async ({
  page,
}) => {
  await connectAndScan(page);
  await page.getByRole("button", { name: "Compare 1 selected asset" }).click();
  ...
  const aave = page.locator('[data-protocol-id="aave"]');
  const spark = page.locator('[data-protocol-id="sparklend"]');
  const aaveToggle = aave.getByRole("button").first();
  await expect(aaveToggle).toHaveAttribute("aria-expanded", "false");
  await expect(aave.locator("[data-market-details]")).toHaveCount(0);
  await aaveToggle.click();
  await expect(aaveToggle).toHaveAttribute("aria-expanded", "true");
  await expect(aave.locator("[data-market-details]")).toBeVisible();
  await expect(
    aave.getByRole("heading", { name: "Technical details" }),
  ).toBeVisible();
  await expect(aave.getByText("Projected LTV", { exact: true })).toBeVisible();
  await expect(aave.getByText("Liquidation threshold")).toBeVisible();
  await spark.getByRole("button").first().click();
  await expect(aave.locator("[data-market-details]")).toHaveCount(0);
  await expect(spark.locator("[data-market-details]")).toBeVisible();
  const morpho = page.locator('[data-protocol-id="morpho-blue"]');
  await morpho.getByRole("button").first().click();
  await expect(spark.locator("[data-market-details]")).toHaveCount(0);
  await expect(morpho.locator("[data-market-details]")).toBeVisible();
  await expect(
    morpho.getByText("Projected LTV", { exact: true }),
  ).toBeVisible();
  await expect(
    morpho.getByText("Liquidation threshold", { exact: true }),
  ).toBeVisible();
});
