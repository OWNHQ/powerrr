...
export const OWN_OPPORTUNITY_POLICY_VERSION = "own-collateral-v1-2026-07-15";
...
const POLICY: Record<string, CollateralPolicy> = {
  WETH: { advanceRate: 0.52, haircut: 0.96, family: "ETH" },
  wstETH: { advanceRate: 0.5, haircut: 0.96, family: "ETH" },
  WBTC: { advanceRate: 0.48, haircut: 0.94, family: "BTC" },
  cbBTC: { advanceRate: 0.46, haircut: 0.94, family: "BTC" },
  USDC: { advanceRate: 0.72, haircut: 0.98, family: "USD" },
  DAI: { advanceRate: 0.72, haircut: 0.98, family: "USD" },
  USDS: { advanceRate: 0.72, haircut: 0.98, family: "USD" },
};
...
export function calculateOwnOpportunity(
  ...
  const indicativeApr = bounded(config.indicativeApr ?? 0.095, 0, 1);
  ...
  for (const asset of portfolio) {
    const policy = POLICY[asset.symbol];
    ...
    const capacity = valueUsd * policy.advanceRate * policy.haircut;
    familyCapacity.set(
      policy.family,
  ...
  const concentrationAdjustedCapacity = [...familyCapacity.values()].reduce(
    (sum, capacity) => sum + Math.min(capacity, maxFamilyContributionUsd),
    0,
  );
  ...
  return {
    ...
    assumptions: [
      ...
      "OWN requests use a fixed duration and time-based default rather than automatic price-triggered liquidation.",
    ],
    warnings:
      potentialBorrowUsd <= 0
        ? [
