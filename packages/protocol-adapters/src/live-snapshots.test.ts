...
describe("live protocol snapshot quote builders", () => {
  ...
  it("prioritizes actual capacity when shared-collateral markets use different oracle prices", () => {
    const token = "0x0000000000000000000000000000000000000001";
    const route = buildMorphoBorrowRoute(
      [
        {
          ...isolatedMarket("higher-lltv", token, 100, 0.9, 100, 0.04),
          oraclePrice: ratio(0.9),
        },
        {
          ...isolatedMarket("higher-capacity", token, 100, 0.8, 44, 0.05),
          oraclePrice: ratio(1.1),
        },
      ],
      1_000n * 10n ** 6n,
      "capacity",
