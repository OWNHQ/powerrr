...
describe("Compound III live Comet snapshot source", () => {
  ...
  it("keeps ETH-only collateral visible when its maximum is below the minimum borrow", async () => {
    const eth = "0xEeeeeEeeeEeEeeEeEeeEEEeeeeEeeeeeeeEEeE" as const;
    const snapshot = await loadCompoundUsdcCometSnapshot({
      rpc: createCompoundRpcMock({
        mode: "wallet-estimate",
      ...
      portfolio: [
        {
          ...
          decimals: 18,
          balance: "0.01",
          balanceRaw: parseUnits("0.01", 18).toString(),
          protocolAssetToken: weth,
          protocolBalanceRaw: parseUnits("0.01", 18).toString(),
          requiredAction: "wrap",
          marketPriceUsd: 3_500,
          protocolEligible: { "compound-iii": true },
        },
      ],
      selectedCollateralTokens: [eth],
      targetBorrowAssets: ["USDC"],
      safetyProfile: "balanced",
    });
    const quote = quoteLiveSnapshots({
      snapshots: projectLiveSnapshots([snapshot], [eth]),
    })[0]!;

    expect(quote.collateralUsed).toEqual([
      expect.objectContaining({ symbol: "ETH", valueUsd: 35 }),
    ]);
    expect(quote.capacityBreakdown?.bindingConstraint).toBe("minimum-borrow");
