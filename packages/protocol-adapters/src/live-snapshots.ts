...
function compareMarketCapacityPriority(
  ...
  if (
    ...
    const capacityComparison = compareRatioProducts(
      right.oraclePrice,
      right.lltv,
      left.oraclePrice,
      left.lltv,
...
function compareRatioProducts(
  leftFirst: RawRatio,
  leftSecond: RawRatio,
  rightFirst: RawRatio,
  rightSecond: RawRatio,
): number {
  const leftNumerator =
    BigInt(leftFirst.numerator) * BigInt(leftSecond.numerator);
  const leftDenominator =
    BigInt(leftFirst.denominator) * BigInt(leftSecond.denominator);
  const rightNumerator =
    BigInt(rightFirst.numerator) * BigInt(rightSecond.numerator);
  const rightDenominator =
    BigInt(rightFirst.denominator) * BigInt(rightSecond.denominator);
