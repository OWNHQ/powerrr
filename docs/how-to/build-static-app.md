...
Clarity is an optional build-time integration and is disabled by default.
...
To enable Clarity, supply its public project ID at build time:

```bash
NUXT_PUBLIC_CLARITY_PROJECT_ID=<project-id> pnpm build:static
```

The project ID may contain only letters and numbers. The bootstrap uses Clarity Consent V2 to deny both
...
as `https://<CID>.ipfs.dweb.link/`; the app uses relative asset paths and is also
compatible with path-style gateways. See
